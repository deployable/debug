...
module NDebug
  ...
  # Scope vars under `d_nodedebug` so we (hopefully) don't interfere with 
  ...
  def self.included klass
    ...
    debugs.map!{|a| a.gsub( /(?<!\.)\*/ , '.*' ) }
    klass.instance_variable_set :@d_nodedebug_these_classes, debugs
    ...
    re = Regexp.new( /
      ...
      \Z                     # End of string
    /x )
    klass.instance_variable_set :@d_nodedebug_this_re, re
    
    # Compare the regex against the current class name and switch
    # debug on if needed.
    match = re.match klass.to_s
    klass.instance_variable_set :@d_nodedebug_class_match_data, match
    klass.instance_variable_set :@d_nodedebug_is_on, !!match

    # Set an initial time for the next ms calculation.
    klass.instance_variable_set :@d_nodedebug_time_last, Time.now.utc

    if klass.instance_variable_get :@d_nodedebug_is_on
      alias_method NDebug::METHOD_NAME, :d_nodedebug_on
    else
      alias_method NDebug::METHOD_NAME, :d_nodedebug_off
    end
  ...
  # Noop when off
  def d_nodedebug_off msg, *args, &block
    false
  end


  # The main `.debug` method
  def d_nodedebug_on msg, *args, &block
    cls = self.class

    # Exit early if we are not needed
    # return false unless cls.instance_variable_get :@d_nodedebug_is_on
    
    # Setup the log array from a block, if given
    args = Array[yield] if block_given?
    
    # Calculate the ms since last log
    time_prev = cls.instance_variable_get( :@d_nodedebug_time_last )
    ...
    # Store the new time
    cls.instance_variable_set :@d_nodedebug_time_last, time_now
